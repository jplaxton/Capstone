...
myStemMat <- dfm(my_corpus, remove = stopwords("english"), stem = TRUE, 
                 remove_punct = TRUE, remove_symbols = TRUE, remove_separators = TRUE)
d1 <- textstat_frequency(myStemMat, n=20)
p <- ggplot(d1, aes(x=reorder(feature, -frequency), y = frequency)) +
        geom_bar(stat = "identity") +
        geom_text(aes(label = frequency), size=2, vjust = -0.2) +
        labs(x = NULL, y = "Frequency", title = "1-Grams") +
        theme(plot.title = element_text(hjust=0.5),
              axis.text.x = element_text(angle=30, hjust = 1, vjust = 1))
p
# versus corpus word frequency with stopwords and no stemming
d3 <- textstat_frequency(dfm(dataTokens), n=20)
p <- ggplot(d3, aes(x=reorder(feature, -frequency), y = frequency)) +
        geom_bar(stat = "identity") +
        labs(x = NULL, y = "Frequency", title = "1-Grams w/ Stop Words and No Stemming") +
        theme(plot.title = element_text(hjust=0.5),
              axis.text.x = element_text(angle=30, hjust = 1, vjust = 1))
p
...
# bigram frequency
d4 <- textstat_frequency(dfm(ngram2),n=20)
p <- ggplot(d4, aes(x=reorder(feature, -frequency), y = frequency)) +
        geom_bar(stat = "identity") +
        labs(x = NULL, y = "Frequency", title = "2-Grams") +
        theme(plot.title = element_text(hjust=0.5), 
              axis.text.x = element_text(angle=30, hjust = 1, vjust = 1))
p

d5 <- textstat_frequency(dfm(ngram3),n=20)
p <- ggplot(d5, aes(x=reorder(feature, -frequency), y = frequency)) +
        geom_bar(stat = "identity") +
        labs(x = NULL, y = "Frequency", title = "3-Grams") +
        theme(plot.title = element_text(hjust=0.5),
              axis.text.x = element_text(angle=30, hjust = 1, vjust = 1))
p
```
...
```{r}
d3 <- textstat_frequency(dfm(dataTokens))
sumWords <- 0
for(i in 1:length(d3$frequency)) {
    sumWords <- sumWords + d3$frequency[i]
    if(sumWords >= 0.5*sum(d3$frequency)){break}
}

sprintf("%d unique words constitue 50 percent of the language", i)

sumWords <- 0
for(i in 1:length(d3$frequency)) {
    sumWords <- sumWords + d3$frequency[i]
    if(sumWords >= 0.9*sum(d3$frequency)){break}
}

sprintf("%d unique words constitue 90 percent of the language", i)
```

```{r uniquewords}
termFreq <- sort(colSums(as.matrix(myStemMat)), decreasing=TRUE) 
